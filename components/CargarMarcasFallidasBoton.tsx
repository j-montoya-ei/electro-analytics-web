// ═══════════════════════════════════════════════════════════
// Botón "Cargar marcas fallidas" (client component)
// Sube el reporte "Intentos de Marcaje" (.xls/.xlsx) a
// /api/marcas-fallidas/cargar, muestra el resumen y refresca.
//
// Ubicación: components/CargarMarcasFallidasBoton.tsx
// ═══════════════════════════════════════════════════════════
...
export default function CargarMarcasFallidasBoton() {
  ...
  async function onArchivo(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const res = await fetch('/api/marcas-fallidas/cargar', {
        method: 'POST',
        body: form,
      })
  ...
  return (
    <div className="flex flex-col items-start gap-2 sm:items-end">
      ...
      >
        ...
        {cargando ? 'Cargando…' : 'Cargar marcas fallidas'}
      </button>
      ...
      {resumen && (
        <div className="flex items-start gap-2 rounded-lg border border-green-200 bg-green-50 px-3 py-2 text-xs text-green-800">
          ...
          <span>
            {resumen.insertados} marca(s) nueva(s) · {resumen.ya_existentes} ya
