// ═══════════════════════════════════════════════════════════
// Botón "Cargar cartera (EPS)" (client component)
// Sube el libro RECAUDO a /api/cartera-eps/cargar (sección 2).
//
// Ubicación: components/CargarCarteraEpsBoton.tsx
// ═══════════════════════════════════════════════════════════
...
export default function CargarCarteraEpsBoton() {
  ...
  async function onArchivo(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const res = await fetch('/api/cartera-eps/cargar', { method: 'POST', body: form })
  ...
  return (
    <div className="flex flex-col items-start gap-2 sm:items-end">
      <button type="button" onClick={() => inputRef.current?.click()} disabled={cargando} className={btnPrimary}>
        {cargando ? <Loader2 className="h-4 w-4 animate-spin" /> : <Upload className="h-4 w-4" />}
        {cargando ? 'Cargando…' : 'Cargar cartera (EPS)'}
      </button>
      ...
      {resumen && (
        <div className="flex items-start gap-2 rounded-lg border border-green-200 bg-green-50 px-3 py-2 text-xs text-green-800">
          ...
          <span>
            {resumen.insertados} nuevo(s) · {resumen.actualizados} actualizado(s) ·{' '}
