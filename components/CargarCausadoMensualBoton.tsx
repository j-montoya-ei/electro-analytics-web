// ═══════════════════════════════════════════════════════════
// Botón "Cargar causado mensual" (client component)
// Sube el libro RECAUDO (.xls/.xlsx) a /api/causado-mensual/cargar,
// muestra el resumen y refresca. Carga solo la matriz mensual
// (MES / INCAPACIDADES / VALOR INGRESO) → tabla causado_mensual.
//
// Ubicación: components/CargarCausadoMensualBoton.tsx
// ═══════════════════════════════════════════════════════════
...
export default function CargarCausadoMensualBoton() {
  ...
  async function onArchivo(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const res = await fetch('/api/causado-mensual/cargar', {
        method: 'POST',
        body: form,
      })
  ...
  return (
    <div className="flex flex-col items-start gap-2 sm:items-end">
      ...
      >
        ...
        {cargando ? 'Cargando…' : 'Cargar causado mensual'}
      </button>
      ...
      {resumen && (
        <div className="flex items-start gap-2 rounded-lg border border-green-200 bg-green-50 px-3 py-2 text-xs text-green-800">
          ...
          <span>
            {resumen.insertados} mes(es) nuevo(s) · {resumen.actualizados}{' '}
